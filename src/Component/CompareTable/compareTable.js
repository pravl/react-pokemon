           ...
           <table className="table">
                <thead className="heading">
                    <tr className="heading-row">
                        <th>Attribute
                            <button className="hintText" onClick={() => this.onPress()}>Edit</button>
                        </th>
                        { CompareList.map((x,idx) => <th key={idx}>{this.getDetail(x, "name")}</th>)}
                    </tr>
                    </thead>
                    <tbody>
                   { attribute.includes("abilities") && <tr>
                        <td className="attribute">Abilities</td>
                        { CompareList.map((x,idx) => <td className="row" key={idx}>{this.getDetail(x, "abilities")}</td>)}
                    </tr>
                   }
                   { attribute.includes("weakness") && <tr>
                        <td className="attribute">Weakness</td>
                        { CompareList.map((x,idx) => <td className="row" key={idx}>{this.getDetail(x, "weakness")}</td>)}
                    </tr>
                   }
                    { attribute.includes("weight") && <tr>
                        <td className="attribute">Weight</td>
                        { CompareList.map((x,idx) => <td  className="row" key={idx}>{this.getDetail(x, "weight")}</td>)}
                    </tr>
                    }
                    { attribute.includes("type") && <tr>
                        <td className="attribute">Type</td>
                        { CompareList.map((x,idx) => <td  className="row" key={idx}>{this.getDetail(x, "type")}</td>)}
                    </tr>
                    }
